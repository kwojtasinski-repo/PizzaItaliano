...
using PizzaItaliano.Services.Products.Tests.Shared.Helpers;
...
namespace PizzaItaliano.Services.Products.Tests.Shared.Fixtures
{
    public class RabbitMqFixture
    {
        ...
        public TaskCompletionSource<TMessage> SubscribeAndGet<TMessage>(string exchange)
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                if (message is null)
                {
                    var t = await Task.Run(() => taskCompletionSource.TrySetCanceled());
                }

                var i = await Task.Run(() => taskCompletionSource.TrySetResult(message));
            };
