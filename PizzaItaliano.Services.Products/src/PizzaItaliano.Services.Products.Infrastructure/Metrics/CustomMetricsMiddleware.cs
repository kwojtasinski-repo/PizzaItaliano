...
namespace PizzaItaliano.Services.Products.Infrastructure.Metrics
{
    internal sealed class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/products")] = Query(typeof(GetProducts).Name),
            [GetKey("POST", "/products")] = Command(typeof(AddProduct).Name),
            [GetKey("PUT", "/products")] = Command(typeof(UpdateProduct).Name)
        };
