...
namespace PizzaItaliano.Services.Releases.Infrastructure.Metrics
{
    internal sealed class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/releases")] = Query(typeof(GetReleases).Name),
            [GetKey("POST", "/releases")] = Command(typeof(AddRelease).Name)
        };
